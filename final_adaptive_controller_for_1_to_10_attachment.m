...
k_final = 2.75; % Final control gain after attachment
zeta_initial = 0.4; % Initial control gain
zeta_final = 0.95; % Final control gain after attachment
D = diag(0.01 * ones(1, 9)); % Control gains
L_dist = [0.1; 0.1; 0.1]; % Disturbance torque
t_span = [0, 500]; % Simulation time span
...
[t, x] = ode45(@(t, x) dynamics(t, x, J_original, J2, L_dist, a, k_initial, k_final, zeta_initial, zeta_final, attachment_time, D), t_span, x0, options);
...
for i = 1:length(t)
    if t(i) < attachment_time
        ...
        V(i) = lyapunov_function(x(i, :)', J2, D);
    end
end
...
subplot(3,1,1);
plot(t, L_c_values(:,1));
title('Control Law L_cx over Time');
xlabel('Time (s)');
ylabel('Control torque (L_cx)');

subplot(3,1,2);
plot(t, L_c_values(:,2));
title('Control Law L_cy over Time');
xlabel('Time (s)');
ylabel('Control torque (L_cy)');

subplot(3,1,3);
plot(t, L_c_values(:,3));
title('Control Law L_cz over Time');
xlabel('Time (s)');
ylabel('Control torque (L_cz)');

%% Function definitions

% Define dynamics equations
function dx = dynamics(t, x, J_original, J2, L_dist, a, k_initial, k_final, zeta_initial, zeta_final, attachment_time, D)
    ...
    if t < attachment_time
        ...
        k = k_final;
        n = zeta_final;
        J = J2; % Change inertia after attachment
